package kata;

/**
 * See https://www.codewars.com/kata/your-order-please
 *
 * Your task is to sort a given string. Each word in the String will contain a single number. This
 * number is the position the word should have in the result.
 *
 * Note: Numbers can be from 1 to 9. So 1 will be the first word (not 0).
 *
 * If the input String is empty, return an empty String. The words in the input String will only
 * contain valid consecutive numbers.
 *
 * For an input: "is2 Thi1s T4est 3a" the function should return "Thi1s is2 3a T4est"
 *
 * your_order("is2 Thi1s T4est 3a") [1] "Thi1s is2 3a T4est"
 *
 * @author mirko
 */
public class OrderOfWords {

    public static String order(String words) {
        if (words.isEmpty()) {
            return "";
        }
        String[] strings = words.split(" ");
        String[] sentence = new String[strings.length];
        for (String word : strings) {
            for (int i = 0; i < word.length(); i++) {
                if (Character.isDigit(word.charAt(i))) {
                    int index = Character.getNumericValue(word.charAt(i)) - 1;
                    sentence[index] = word;
                    break;
                }
            }
        }
        return String.join(" ", sentence);
    }

}
